...
if __name__ == '__main__':
    ...
    # 准备llm
    llm, sampling_params, tokenizer = prepare_llm(args=args)
    ...
    for prompt in tqdm(all_prompts):
        messages = [{'role': 'system', 'content': 'You are an helpful AI assistant '}, {'role': 'user', 'content': prompt}]
        ...
        a = tokenizer.batch_decode(generated_ids, skip_special_tokens=True)
        all_outputs.append({"prompt":prompt, "text":tokenizer.batch_decode(generated_ids, skip_special_tokens=True)[0]})
    ...
    for item_idx, (labeled_sample,
        ...
        original_scm_generated_text = original_scm_output["text"][0]  # 从输出对象中获取生成的文本
        hard_intervention_prompt = hard_intervention_output["prompt"]  # 获取原始的输入提示
        hard_intervention_generated_text = hard_intervention_output["text"][0]  # 从输出对象中获取生成的文本
